    ...
    if (!isError()) {
        ...
        case STATE_HEADERS_FINISHED: // Handle body
        {
            ...
            if (resolvedResource.didMatch()) {
                ...
                */
                HTTPRequest *req = new HTTPRequest(this, _httpHeaders, resolvedResource.getMatchingNode(), _httpMethod, resolvedResource.getParams(), _httpResource);
                ...
                if (!req->requestComplete()) {
                    ...
                    // NEW: Check if res->Save() was invoked
                    if (res->isSaved()) {
                        // Detach state handling to keep socket open and avoid automatic destruction
                        _connectionState = STATE_UNDEFINED;
                    } else {
                        // Original cleanup code wrapped in this else block...
                        ...
                        // NEW: Free heap allocations if Save() was NOT called
                        delete req;
                        delete res;
                    }
                }
